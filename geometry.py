...
class Aero():
    ...
    def read_eigen(plane,case):
        with open(case.results_file,'r') as file:
            ...
            try:
                modes={"roll":tuple(map(float,lines[3].split()[1:])),
                        "dutch":tuple(map(float,lines[4].split()[1:])),
                        "short":tuple(map(float,lines[5].split()[1:])),
                        "spiral":tuple(map(float,lines[6].split()[1:])),
                        "phugoid1":tuple(map(float,lines[7].split()[1:])),
                        "phugoid2":tuple(map(float,lines[8].split()[1:]))
                        }
            except IndexError:
