...
class Iterate():
    ...
    def thickness_ratio_12(self,cl_increment):
        ...
        return (-b+np.sqrt(b**2-4*a*c))/(2*a)   # quadratic equation
    ...
    def iterate(self,chord_ratio_initial:float,convergence_criteria:float,
        ...
        while True:
            ...
            data_left=kf_data[kf_data["cf/c"]==ratio_left][["Angle","Kf"]].to_numpy()
            data_right=kf_data[kf_data["cf/c"]==ratio_right][["Angle","Kf"]].to_numpy()

            Kf_left,parameters_left=self.curve_fit(data_left,angle)
            Kf_right,parameters_right=self.curve_fit(data_right,angle)

            Kf.append(np.interp(chord_ratio[n],(ratio_left,ratio_right),(Kf_left,Kf_right)))
            #print(Kf_left,Kf[n],Kf_right)

            Cl_increment.append(dCl/(0.9*Kf[n]*wet_area_ratio*np.deg2rad(angle)*np.cos(np.deg2rad(sweep))))
            chord_ratio.append(self.thickness_ratio_12(Cl_increment[n]))
            #print(chord_ratio[n])
            ...
            #print(chord_ratio[n],chord_ratio[n+1])
...
if __name__=="__main__":
    ...
    chord_ratios=iterate(chord_chord_ratio_initial,convergence,angle,kf_data,dCl,wet_area_ratio,sweep)
